...
```
 user query
    ...
    ▼
 Phase 1 DECOMPOSE   Haiku guardrail + entity/intent extraction; deterministic
 ...
 Phase 4 SYNTHESIZE  deterministic EV/edge math (src/hailmary/synthesis/edge_math.py)
    │                + grounded Sonnet prose + citation guard (citation_guard.py)
    ▼
...
on the fixture's virtual clock. With `REPLAY_LLM=true`, Haiku/Sonnet/Voyage calls replay
from committed cassettes (`fixtures/synthetic_v0/llm_cassettes/`) keyed by SHA-256 of
(model, prompt_version, prompt) — a prompt change fails loudly instead of silently
calling the live API (`src/hailmary/clients/cassette.py`). CI runs the full E2E pipeline
this way with no secrets.
...
Secrets (`ANTHROPIC_API_KEY`, `VOYAGE_API_KEY`, `CFBD_API_KEY`, `ODDS_API_KEY`) are only
needed for live mode and for re-recording cassettes (`scripts/record_cassettes.py`);
...
```bash
uv run pytest -m unit -q            # 245 tests, pure logic, no services needed
...
├── clients/            # postgres, es, qdrant, redis, llm, voyage, cassette replay
...
tests/                  # unit/ (245 tests) · integration/ · e2e/
...
decomposition, synthesis + graph, delivery + dashboard. M8 (live-mode feed clients on
real season data) is the remaining milestone and is season-dependent.
